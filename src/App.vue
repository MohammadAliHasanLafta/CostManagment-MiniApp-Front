  ...
  <div id="app" class="background-container min-h-screen bg-gradient-to-center from-[#FFAB49] to-[#F5811E] flex items-center justify-center p-4">
    ...
    <div class="flex flex-col w-full">
      ...
      <nav class="fixed bottom-0 right-0 w-full shadow-lg flex justify-around items-center py-4 text-gray-600 border-t border-gray-200">
        ...
        <router-link to="/income" class="flex flex-col items-center text-center text-sm" active-class="text-[#FF8100]">
        ...
        <router-link to="/report" class="flex flex-col items-center text-center text-sm" active-class="text-[#FF8100]">
...
>
  ...
  >
    <div class="text-center mb-4">
      <h3 class="text-xl font-semibold">اضافه کردن هزینه</h3>
    ...
    <form @submit.prevent="handleSubmit">
      <div class="mb-4">
        <label for="expense-title" class="block text-gray-600 font-semibold pb-2" dir="rtl">موضوع هزینه</label>
        <input
          v-model="newExpenseTitle"
          id="expense-title"
          type="text"
          placeholder="هزینه جدید..."
      ...
      <div class="mb-4">
        <label for="expense-amount" class="block text-gray-600 font-semibold pb-2" dir="rtl">مبلغ هزینه</label>
        <input
          v-model="newExpenseAmount"
          id="expense-amount"
          type="number"
          placeholder="lمقدار هزینه..."
          class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#F37F00]"
          required
          dir="rtl"
        />
...
export default {
  ...
  methods: {
    ...
    async checkAndRequestContact() {
      ...
      try {
        const response = await axios.get(`https://costmanagementminiapp.runasp.net/get-miniappuser-mobile?UserId=${this.userId}`, {
          headers: {
            'accept': '*/*'
          }
        });
    ...
    async verifyContact() {
      try {
        const response = await axios.post('https://costmanagementminiapp.runasp.net/verify-contact', {
          userId: this.userId,
          mobile: this.mobile,
    ...
    async handleSubmit() {
      ...
      try {
        ...
        this.expenses.push(this.loadData());
    ...
    async requestOtp() {
      try {
        this.click = true; 
        await axios.post("https://costmanagementminiapp.runasp.net/send-otp", {
    ...
    async verifyOtp() {
      let response = "";
      try {
        response = await axios.post('https://costmanagementminiapp.runasp.net/verify-otp', {
          phoneNumber: this.phoneNumber,
    ...
    async sendUserInfoToApi() {
      let response = "";
      try {
        response = await axios.post("https://costmanagementminiapp.runasp.net/verify-initdata", {
          userId: this.userId,
